...
const Navbar = () => {
  ...
  const handlePhoneNumberChange = async (e) => {
    ...
    if (inputNumber.length === 10) { 
      try {
        const response = await fetch(`${config.apiUrl}/check-user/?number=${inputNumber}`, {
          // method: 'GET',
    ...
    <>
      <nav className={`text-orange-500 p-4 sticky top-0 z-50 transition-shadow duration-300 ${isScrolled ? 'shadow-lg bg-white' : 'bg-white'}`}>
        <div className="container mx-auto flex justify-between items-center px-6 md:px-12">
          ...
          <div className={`md:flex md:items-center md:space-x-8 absolute md:static top-16 left-0 w-full bg-white md:bg-transparent shadow-md md:shadow-none md:w-auto md:block transition-all duration-300 ${isOpen ? 'block' : 'hidden'}`}>
